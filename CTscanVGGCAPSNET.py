...
VGGCapsNetmodel.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
from keras.callbacks import ReduceLROnPlateau, ModelCheckpoint, EarlyStopping
...
hist=VGGCapsNetmodel.fit(x=X_train, y=y_train, batch_size=16, epochs=100, validation_data=(X_cv,y_cv),callbacks=callback_list)


epochs=100


#VGGCapsNetmodel.compile(loss='binary_crossentropy', optimizer=SGD(lr=lr,decay=0.001, momentum=0.9), metrics=['accuracy'])

#hist=VGGCapsNetmodel.fit(x=X_train, y=y_train,batch_size=32, epochs=100, validation_data=(X_cv,y_cv), callbacks=callback_list)
...
predicted_classes = VGGCapsNetmodel.predict(X_test)
...
n_classes=2
lw=2
preds = VGGCapsNetmodel.predict(X_test)
y_score = preds
target=y_test
fpr = dict()
tpr = dict()
...
plt.title('ROC curve for Covid classification using VGGCapsNet')
