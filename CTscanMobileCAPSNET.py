...
base_model = MobileNet(include_top=False, weights='imagenet', input_tensor=Input(shape=(128,128,3)))
...
MobileCapsNetmodel = Model(base_model.input, outputs=output)

MobileCapsNetmodel.summary()

train_batch = 8
val_batch = 2

optimizer = Adam(lr=0.0015, beta_1=0.9, beta_2=0.999, epsilon=0.1, decay=0.0)
MobileCapsNetmodel.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
from keras.callbacks import ReduceLROnPlateau, ModelCheckpoint, EarlyStopping
...
hist=MobileCapsNetmodel.fit(x=X_train, y=y_train, batch_size=16, epochs=100, validation_data=(X_cv,y_cv),callbacks=callback_list)


epochs=100


#MobileCapsNetmodel.compile(loss='binary_crossentropy', optimizer=SGD(lr=lr,decay=0.001, momentum=0.9), metrics=['accuracy'])

#hist=MobileCapsNetmodel.fit(x=X_train, y=y_train,batch_size=32, epochs=100, validation_data=(X_cv,y_cv), callbacks=callback_list)
...
plt.title('Training and validation accuracy curves for MobileCapsnet Model')
plt.legend()
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.grid(True)
...
plt.title('Training and validation loss curves for MobileCapsNet Model')
plt.legend()
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.grid(True)
...
predicted_classes = MobileCapsNetmodel.predict(X_test)
...
n_classes=2
lw=2
preds = MobileCapsNetmodel.predict(X_test)
y_score = preds
target=y_test
fpr = dict()
tpr = dict()
...
plt.title('ROC curve for for Covid classification using MobileCapsNet')
